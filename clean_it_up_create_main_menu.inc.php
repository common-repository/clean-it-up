...
function clean_it_up_remove_revision_validate_options($input) {
	...
	if ( ! isset( $input['remove_non_attach_attachment'] ) )
		$input['remove_non_attach_attachment'] = null;
	$input['remove_non_attach_attachment'] = ( $input['remove_non_attach_attachment'] == 1 ? 1 : 0 );
	
	if($input['remove_non_attach_attachment'] == 1) {
		$query = $wpdb->prepare(
	            "
	            DELETE a,b
                FROM $wpdb->posts a 
	            ...
	            WHERE a.post_type = %s
	            AND a.post_parent = %d", 'attachment', 0 );
   	    $wpdb->query( $query );
	}
	...
	if($input['remove_transient'] == 1) {
		$wpdb->query($wpdb->prepare("DELETE FROM $wpdb->options WHERE option_name LIKE %s ", '%\_transient\_%'));
	}
	...
	if ( ! isset( $input['remove_comment_link'] ) )
		$input['remove_comment_link'] = null;
	$input['remove_comment_link'] = ( $input['remove_comment_link'] == 1 ? 1 : 0 );
	
	if($input['remove_comment_link'] == 1) {
		
		$wpdb->query($wpdb->prepare("UPDATE $wpdb->comments SET comment_author_url = %s WHERE comment_author_url <> ''", ""));
	}
...
function clean_it_up_settings_page() {
	...
	$nonpostattachment_count = "SELECT COUNT(post_type) FROM $wpdb->posts WHERE post_type = 'attachment' AND post_parent = 0";
	$post_nonpostattachment_count = $wpdb->get_var($nonpostattachment_count);
...
?>
	<div class="wrap">
		<div class="intro-clean-it-up">
			...
			will have more storage spaces and thus will speed up the loading time of your blog. For further information on each item below please visit the <a href="https://wordpress.org/plugins/clean-it-up/" target="_blank">plugin page</a>. 
		</div>
		...
		<form action="options.php" method="post">
		   <div class="clean-it-up-body">
			...
			<input name='clean_it_up_remove_revision_options[remove_non_attach_attachment]' type="checkbox" value="1" <?php checked( '1', $options['remove_non_attach_attachment'] ); ?> />
			<label for="clean_it_up_remove_revision_options[remove_non_attach_attachment]"><?php _e( "$post_nonpostattachment_count non attached file(s)" ); ?></label><br/>
			...
			<input name='clean_it_up_remove_revision_options[remove_comment_link]' type="checkbox" value="1" <?php checked( '1', $options['remove_comment_link'] ); ?> />
			<label for="clean_it_up_remove_revision_options[remove_comment_link]"><?php _e( "Remove all comment links" ); ?></label><br/>
		   ...
		   <input name='clean_it_up_remove_revision_options[repair_table]' type="checkbox" value="1" <?php checked( '1', $options['repair_table'] ); ?> />
			<label for="clean_it_up_remove_revision_options[repair_table]" class="clean-it-up-table"><?php _e( "Repair database tables (If any table is corrupted, then run this feature)" ); ?></label><br/>
		   </div>
